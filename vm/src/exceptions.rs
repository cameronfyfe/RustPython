...
#[cfg(all(any(target_arch = "wasm32", feature = "no-os"), not(target_os = "wasi")))]
pub(crate) fn raw_os_error_to_exc_type(
    _errno: i32,
    _vm: &VirtualMachine,
) -> Option<&'static Py<PyType>> {
...
pub(super) mod types {
    use crate::common::lock::PyRwLock;
    #[cfg_attr(any(target_arch = "wasm32", feature = "no-os"), allow(unused_imports))]
    use crate::{
        builtins::{
    ...
    impl PyOSError {
        ...
        #[cfg(any(target_arch = "wasm32", feature = "no-os"))]
        #[pyslot]
        fn slot_new(cls: PyTypeRef, args: FuncArgs, vm: &VirtualMachine) -> PyResult {
            PyBaseException::slot_new(cls, args, vm)
        }
